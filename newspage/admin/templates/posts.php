...
if ($user)
{
    ...
    if ($ac != '') 
    {
        ...
        else if ($ac == 'edit')
        {
            ...
            {
                $data_post = $db->db_fetch_assoc($sql_check_id_cate, 1);
                ...
                if ($data_post['author_id'] == $data_user['id_acc'] || $data_user['position'] == '1')
                {
                    ...
                    $data_post = $db->db_fetch_assoc($sql_get_data_post, 1);
                    echo
                    '
                    ...
                    if ($db->db_num_rows($sql_get_cate_post_1)) {
                        if ($data_post['cate_1_id'] == '0') {
                            echo '<option value="">Vui lòng chọn chuyên mục</option>';
                        }
                        foreach ($db->db_fetch_assoc($sql_get_cate_post_1, 0) as $key => $data_cate_1) {
                    ...
                    if ($db->db_num_rows($sql_get_cate_post_2)) {
                        if ($data_post['cate_2_id'] == '0') {
                            echo '<option value="">Vui lòng chọn chuyên mục</option>';
                        }
                        foreach ($db->db_fetch_assoc($sql_get_cate_post_2, 0) as $key => $data_cate_2) {
                    ...
                    if ($db->db_num_rows($sql_get_cate_post_3)) {
                        if ($data_post['cate_3_id'] == '0') {
                            echo '<option value="">Vui lòng chọn chuyên mục</option>';
                        }
                        foreach ($db->db_fetch_assoc($sql_get_cate_post_3, 0) as $key => $data_cate_3) {
    ...
    else
    {
        ...
        // Nếu có bài viết
        if ($db->db_num_rows($sql_get_list_post))
        {
            // Lấy số trang
            if (isset($_GET['page'])) {
            ...
            $total_page = ceil($db->db_num_rows($sql_get_list_post) / $limit); // Tổng trang
            ...
            foreach ($db->db_fetch_assoc($sql_get_list_post_limit, 0) as $key => $data_post) 
            {
                // Trạng thái bài viết
                ...
                if ($db->db_num_rows($sql_check_id_cate_1)) {
                    $data_cate_1 = $db->db_fetch_assoc($sql_check_id_cate_1, 1);
                ...
                if ($db->db_num_rows($sql_check_id_cate_2)) {
                    $data_cate_2 = $db->db_fetch_assoc($sql_check_id_cate_2, 1);
                ...
                if ($db->db_num_rows($sql_check_id_cate_3)) {
                    $data_cate_3 = $db->db_fetch_assoc($sql_check_id_cate_3, 1);
                ...
                if ($db->db_num_rows($sql_get_author)) {
                    $data_author = $db->db_fetch_assoc($sql_get_author, 1);
