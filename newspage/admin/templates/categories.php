                        ...
                        <form method="POST" id="formAddCate" onsubmit="return false;">
                            ...
                            <div class="form-group hidden parent-add-cate">
                                ...
                                <select id="parent_add_cate" class="form-control">
                                    <option>Tự chọn 1</option>
                                    <option>Tự chọn 2</option>
                                    <option>Tự chọn 3</option>
                                </select>
                            </div>
                            <div class="form-group">
				...
				if ($db->db_num_rows($sql_check_id_cate)) 
				{
                    ...
                    if ($db->db_num_rows($sql_get_data_cate))
                    {
                        ...
                        else if ($data_cate['type'] == 2)
                        {
                            ...
                            if ($db->db_num_rows($sql_get_cate_parent))
                            {
                                // In danh sách các chuyên mục cha loại 1
                                foreach ($db->db_fetch_assoc($sql_get_cate_parent, 0) as $key => $data_cate_parent)
                                {
		...
		else
		{
            ...
            if ($db->db_num_rows($sql_get_list_cate))
            {
                ...
                foreach ($db->db_fetch_assoc($sql_get_list_cate, 0) as $key => $data_cate)
                {
                    ...
                    if ($db->db_num_rows($sql_get_cate_parent))
                    {
                        $data_cate_parent = $db->db_fetch_assoc($sql_get_cate_parent, 1);
